...
static vector<SMaterial> LoadMaterialsFromFile(const aiScene* Scene, const string& FileName)
{
	...
	for (unsigned int iMaterial{}; iMaterial < MaterialCount; ++iMaterial)
	{
		...
		else
		{
			...
			string s; // ñþäà áóäåì êëàñòü ñ÷èòàííûå ñòðîêè
			std::ifstream file(mtlfile); // ôàéë èç êîòîðîãî ÷èòàåì (äëÿ ëèíóêñ ïóòü áóäåò âûãëÿäåòü ïî äðóãîìó)

			while (getline(file, s)) { // ïîêà íå äîñòèãíóò êîíåö ôàéëà êëàñòü î÷åðåäíóþ ñòðîêó â ïåðåìåííóþ (s)
	...
	for (unsigned int iMesh = 0; iMesh < Model.vMeshes.size(); ++iMesh)
	{
		...
		if (aiMesh->HasBones())
		{
			for (unsigned int iBone = 0; iBone < aiMesh->mNumBones; ++iBone)
			{
				...
				// BoneCount´Â ÇöÀç BoneÀÌ Ã³À½ ³ª¿Â °æ¿ì¿¡¸¸ Áõ°¡½ÃÄÑ¾ß ÇÑ´Ù! (µ¿ÀÏÇÑ BoneÀÌ ¼­·Î ´Ù¸¥ Mesh¿¡¼­ ÂüÁ¶µÉ ¼ö ÀÖ±â ¶§¹®)
				...
				// ¿©·¯ Mesh°¡ °°Àº BoneÀ» ÂüÁ¶ÇÒ ¼ö ÀÖÀ¸¹Ç·Î ¾Æ·¡ ÄÚµå´Â ¸Å¹ø ½ÇÇàµÇ¾î¾ß ÇÑ´Ù.

...
/// <summary>
/// Конвертирует вектор в строку
/// </summary>
/// <param name="Vector">Конвертируемый вектор</param>
...
static vector<STriangle> GenerateContinuousFacesReverse(int FaceCount)
{
	...
	for (int FaceIndex = 0; FaceIndex < FaceCount; ++FaceIndex)
	{
		vTriangles.emplace_back(FaceIndex * 4 + 0, FaceIndex * 4 + 2, FaceIndex * 4 + 1);
		vTriangles.emplace_back(FaceIndex * 4 + 1, FaceIndex * 4 + 2, FaceIndex * 4 + 3);
	}

	return vTriangles;
}

#pragma region Generate2DObjectMethods
...
#pragma region Generate3DObjectMethods
